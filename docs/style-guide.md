...
- **Booleans read as questions** (`isValid`, `hasMitreId`); **functions are verbs** (`loadColumns`, `validateContent`).
...
### Matrix techniques (`content/matrix/columns/NN-*.yaml`)
Each column file lists techniques in priority order. A technique is:
...
- **`label`** — concise, specific, sentence-case noun phrase describing the behavior. Match the vocabulary already used in the column. Avoid vendor product names unless the technique is named for one (e.g., "Help-desk SE (Scattered Spider)").
...
- Order reflects prominence/representativeness within the column, mirroring the source workbook.

### Phases (`content/matrix/phases.yaml`)
The five phases are fixed (Internal, Approach, Deception, Imposition, Alignment). Edits here are rare and require strong justification — phase boundaries are a structural decision of the taxonomy.
...
- **Cross-link** every essay to the matrix columns it maps to.
...
- **Conventional Commits**: `type(scope): summary` — e.g., `feat(matrix): add column detail panel`, `content(column-07): add quishing technique`, `fix(ci): pin actions to SHAs`, `docs(roadmap): clarify phase exits`. Types: `feat`, `fix`, `content`, `docs`, `refactor`, `test`, `chore`, `ci`.
